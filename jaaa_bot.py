"""
jaaa_bot.py — Monitors the JAAA pool on Centrifuge and sends Telegram alerts.
...
POOL_ID            = os.getenv("POOL_ID", "281474976710663")
...
log = logging.getLogger("jaaa_bot")
...
def check_deposit_requests(bot: Bot, state: dict) -> None:
    ...
    seen  = set(state["seen_deposit_requests"])
    for node in reversed(nodes):
        ...
        if tx not in seen:
            ...
            write_lifecycle_record(state, "JAAA", node, node["type"])
    state["seen_deposit_requests"] = list(seen)
...
def check_deposit_claimable(bot: Bot, state: dict) -> None:
    ...
    if not data:
        return
    nodes = data.get("investorTransactions", {}).get("items", [])
    seen  = set(state["seen_deposit_claimable"])
    for node in reversed(nodes):
        tx = node["txHash"]
        if tx not in seen:
            seen.add(tx)
            ...
            write_lifecycle_record(state, "JAAA", node, node["type"])
    state["seen_deposit_claimable"] = list(seen)
...
def check_redeem_requests(bot: Bot, state: dict) -> None:
    ...
    if not data:
        return
    nodes = data.get("investorTransactions", {}).get("items", [])
    seen  = set(state["seen_redeem_requests"])
    for node in reversed(nodes):
        tx = node["txHash"]
        if tx not in seen:
            seen.add(tx)
            ...
            write_lifecycle_record(state, "JAAA", node, node["type"])
    state["seen_redeem_requests"] = list(seen)
...
def check_redeem_claimable(bot: Bot, state: dict) -> None:
    ...
    if not data:
        return
    nodes = data.get("investorTransactions", {}).get("items", [])
    seen  = set(state["seen_redeem_claimable"])
    for node in reversed(nodes):
        tx = node["txHash"]
        if tx not in seen:
            seen.add(tx)
            ...
            write_lifecycle_record(state, "JAAA", node, node["type"])
    state["seen_redeem_claimable"] = list(seen)
...
def main() -> None:
    ...
    send_telegram(bot, "🟢 *JAAA bot is online*\nMonitoring pool `281474976710663` — polling every 60s")
    log.info("JAAA bot started. Pool ID: %s — polling every %ds", POOL_ID, POLL_INTERVAL)
    ...
    while True:
        ...
        try:
            ...
            send_telegram(bot, f"🔴 *JAAA bot error*\n```{exc}```")
